...
## Project Specialists (RedPower LP)

Este projeto possui agentes especialistas próprios em `.claude/agents/` (via Agent tool, `subagent_type`). Prefira-os aos agentes genéricos para trabalho de domínio:

| Agente | Domínio |
|--------|---------|
| `redpower-frontend` | LP, seções, design system dark (tokens, Glancyr/Figtree), Tailwind v4, React 19 |
| `redpower-checkout` | Jornada de compra: Kiwify, /envio, UTMs, webhook, planos/preços, Redrive API V2 |
| `redpower-metodo` | Plataforma do aluno /metodo: Vimeo, gatilhos do agente, notas, backend planejado |
| `redpower-data` | Supabase (vendas, pre_checkout), RLS, /api/vendas, /api/rp-ingest, painel /ops |
| `redpower-infra` | Jenkins, kaniko/Harbor, k8s namespace `marketing`, release commit-and-tag-version |
| `redpower-copy` | Copy de vendas, catálogo/garantias (Doc de Produto v3), tom de voz, FAQ |

Contexto consolidado do produto/checkout: `docs/07-checkout-contexto-descobertas.md`.
